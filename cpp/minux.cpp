...
void handle_command(const char *cmd) {
    ...
    else if (strcmp(args[0], "wav") == 0) {
        if (argc > 1) {
            // Get the original command string and extract everything after "wav "
            const char *wav_arg = cmd + 4;
            while (isspace(*wav_arg)) wav_arg++;
            cmd_wav(wav_arg);
        } else {
            log_error(error_console, ERROR_WARNING, "MINUX", 
                     "Usage: wav [record <name> | list | play <name>]");
        }
        show_prompt();
    }
    else {
    ...
    for (int i = 1; i <= len1; i++) {
        for (int j = 1; j <= len2; j++) {
            if (s1[i-1] == s2[j-1]) {
                ...
                matrix[i][j] = 1 + std::min(std::min(matrix[i-1][j], matrix[i][j-1]), matrix[i-1][j-1]);
            }
        }
    }
...
static int levenshtein_distance(const char *s1, const char *s2) {
    ...
    // Initialize the matrix (rest of the function)
    for (int i = 0; i <= len1; i++) {
        matrix[i][0] = i;
    }
...
// WAV command prototypes
void cmd_wav(const char *arg);
void record_wav(const std::string &filename, float duration_sec = 3.0);
void list_wav_files();
...
void play_midi_test(void) {
    ...
    // Create a MIDI client
    status = MIDIClientCreate(CFSTR("MINUX MIDI Test"), NULL, NULL, &midiclient);
    if (status != noErr) {
        printw("\nError creating MIDI client: %d\n", (int)status);
        refresh();
        return;
    ...
    status = MIDIOutputPortCreate(midiclient, CFSTR("MINUX Output Port"), &outPort);
    if (status != noErr) {
        printw("\nError creating output port: %d\n", (int)status);
        refresh();
        MIDIClientDispose(midiclient);
        return;
    }
    
    // Get the first available MIDI destination
    ItemCount destCount = MIDIGetNumberOfDestinations();
    if (destCount == 0) {
    ...
    if (destination == 0) {
        printw("\nCouldn't get default MIDI destination\n");
        refresh();
        MIDIPortDispose(outPort);
        MIDIClientDispose(midiclient);
        return;
    ...
    char nameCStr[256] = "unknown";
    if (name != NULL) {
        CFStringGetCString(name, nameCStr, sizeof(nameCStr), kCFStringEncodingUTF8);
        CFRelease(name);
    }
    
    printw("\nPlaying Mozart's Turkish March (simplified) on %s...\n", nameCStr);
    refresh();
    ...
    struct {
        ...
        {0x90, 0x3C, 0x7F}, // Note on, middle C
        {0x90, 0x40, 0x7F}, // Note on, E
        {0x90, 0x43, 0x7F}, // Note on, G
        {0x90, 0x48, 0x7F}, // Note on, C (octave up)
        {0x90, 0x43, 0x7F}, // Note on, G
        {0x90, 0x40, 0x7F}, // Note on, E
        {0x90, 0x3C, 0x7F}, // Note on, middle C
    };
    
    int noteCount = sizeof(noteMessages) / sizeof(noteMessages[0]);
    
    // Play each note
    for (int i = 0; i < noteCount; i++) {
        // Create a MIDI packet list with one packet
        MIDIPacketList packetList;
        MIDIPacket *curPacket = MIDIPacketListInit(&packetList);
        Byte midiData[3] = {noteMessages[i].status, noteMessages[i].note, noteMessages[i].velocity};
        MIDITimeStamp timestamp = 0; // Send immediately
        
        curPacket = MIDIPacketListAdd(&packetList, sizeof(packetList), curPacket, timestamp, 3, midiData);
        if (curPacket == NULL) {
            printw("Error creating MIDI packet\n");
            continue;
        }
        
        // Send the packet list
        status = MIDISend(outPort, destination, &packetList);
        if (status != noErr) {
            printw("Error sending MIDI message: %d\n", (int)status);
        }
        
        // Wait for 500 ms
        usleep(500000);
        
        // Note off (same note with velocity 0)
        MIDIPacketList offPacketList;
        MIDIPacket *offPacket = MIDIPacketListInit(&offPacketList);
        Byte offData[3] = {0x80, noteMessages[i].note, 0x00};
        
        offPacket = MIDIPacketListAdd(&offPacketList, sizeof(offPacketList), offPacket, timestamp, 3, offData);
        if (offPacket != NULL) {
            MIDISend(outPort, destination, &offPacketList);
        }
